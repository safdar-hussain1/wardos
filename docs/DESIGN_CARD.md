...
| C4 | Money never floats | Every amount is an integer number of paise end to end; `requirePaise` rejects fractions, negatives, NaN and unsafe integers in the three commands that take an amount from the interface — `admit` (deposit), `recordDeposit`, `addCharge` — before any transaction opens. Staff base pay, set only by the seed through `addStaff`, relies on the schema's `CHECK (base_paise > 0)` instead | Removed the `requirePaise` check from `addCharge` in `src/core/engine.ts` | 2 tests, 1 file — `amountPaise=1.5` and `amountPaise=NaN` must be rejected with no partial state |
