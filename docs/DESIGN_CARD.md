...
| C4 | Money never floats | Every amount is an integer number of paise end to end; `requirePaise` rejects fractions, negatives, NaN and unsafe integers in every command that takes an amount — `admit` (deposit), `recordDeposit`, `addCharge`, `addStaff` (base pay) — before any transaction opens | Removed the `requirePaise` check from `addCharge` in `src/core/engine.ts` | 2 tests, 1 file — `amountPaise=1.5` and `amountPaise=NaN` must be rejected with no partial state |
...
Each mutation was a temporary edit: apply, run the full suite, record the failing test names, revert, re-run the covering files to confirm green. The killer counts above come from those runs. The same boundary check in `addStaff` is covered the same way: removing it fails 3 tests in 1 file (`base_paise` of 1.5, −100 and NaN must each be rejected before the transaction opens).
